...
class OWNNorm(torch.nn.Module):
    ...
    def forward(self, weight: torch.Tensor):
        #self.norm_groups = int(weight.shape[0] / 64)
        ...
        for i in range(self.norm_groups):
            # Changed because of convergence issue
            try:
                U, Eig, _ = S[i].svd()
            except:                     # torch.svd may have convergence issues for GPU and CPU.
                U, Eig, _ = torch.svd(S[i] + 1e-8*S[i].mean()*(torch.rand(S[i].shape).to(S.device)))
...
class OWN_Conv2d(torch.nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
                 norm_groups=64, NScale=1.414, adjustScale=False):
        #bias = True changed norm_groups to 64
        super(OWN_Conv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        print('OWN_conv:----norm_groups:', norm_groups, '---NScale:', NScale, '---adjust:', adjustScale, '--bias', bias)
